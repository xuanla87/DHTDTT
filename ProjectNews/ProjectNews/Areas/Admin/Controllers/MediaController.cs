...
namespace ProjectNews.Areas.Admin.Controllers
{
    ...
    public class MediaController : Controller
    {
        ...
        {
            ContentView result;
            result = _services.GetAll(_searchKey, _fromDate, _toDate, _parentId, "MEDIA", 1, false, _pageIndex, 20);
        ...
        {
            ContentView result;
            result = _services.GetAll(_searchKey, null, null, _parentId, "CHUYENMUCMEDIA", 1, false, _pageIndex, 20);
            int totalPage = result?.Total ?? 0;
            ViewBag.TotalPage = totalPage;
            ViewBag.PageIndex = _pageIndex ?? 1;
            ViewBag.SearchKey = string.IsNullOrWhiteSpace(_searchKey) ? string.Empty : _searchKey;
